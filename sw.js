// Service Worker Seguro para ServiYa.do 🇩🇴 PWA
// Versión: 1.0.0
  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      return cache.addAll(PUBLIC_ASSETS).catch((err) => {
        console.warn('ServiYa.do 🇩🇴 SW: Algunos activos públicos no se pudieron precachear:', err);
      });
    })
  );
  self.skipWaiting();
  ...
  if (url.pathname.startsWith('/api/') || url.pathname.includes('/auth/') || url.pathname.includes('/pagos/') || url.pathname.includes('/admin/')) {
    event.respondWith(
      fetch(request).catch(() => {
        return new Response(
          JSON.stringify({ 
            error: 'Sin conexión a internet. Esta acción requiere conexión al servidor seguro de ServiYa.do 🇩🇴.' 
          }),
          { 
            status: 503, 
