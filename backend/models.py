...
class Chamba(Base):
    ...
    # Estados: publicada, recibiendo_postulaciones, trabajador_seleccionado, en_progreso, completada, cancelada, en_disputa
...
class Postulacion(Base):
    ...
    # Required verification booleans
    can_perform = Column(Boolean, default=False)
    has_tools = Column(Boolean, default=False)
    available_on_date = Column(Boolean, default=False)
    needs_client_supplies = Column(Boolean, default=False)
    confirm_details_read = Column(Boolean, default=False)
...
class Contract(Base):
    ...
    total_client_amount_rd = Column(Float, default=0.0)
